function init()
{
    ...
    player1.graphic.position.x = player1.position.x;
    player1.graphic.position.y = player1.position.y;
    scene.add(player1.graphic);

    player2 = new Ennemy("player2", 0xff0000, new THREE.Vector2(-50, 0), 0);
    player2.graphic.position.x = player2.position.x;
    player2.graphic.position.y = player2.position.y; 
...
function Ground(color, size_x, size_y, nb_tile)
{
    ...
    only_colors = Array(0xff0000, 0x00ff00, 0x0000ff);
    ...
    for (x = minX; x <= maxX; x = x+sizeOfTileX){
        for (y = minY; y <= maxY; y = y+sizeOfTileY){
            ...
            if (x === 0 && y === 0) {
                color = only_colors[Math.floor(Math.random()*only_colors.length)];
            }
